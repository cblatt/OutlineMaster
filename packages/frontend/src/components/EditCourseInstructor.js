...
export default function EditCourseInstructor() {
  ...
  const handleSelectChange = (e) => {
    ...
    setCourseUuid(uuid);
    setVersionNum(num);
    submitChanges(uuid, num);
  };

  function submitChanges(id, version) {
    fetch(
  ...
  return (
    <div>
      ...
      <div class="min-h-screen bg-gradient-to-r from-purple-500 to-purple-300 ... text-gray-500 py-6 flex flex-col justify-center sm:py-12">
        ...
        <h2 className="text-3xl font-serif text-black mt-4">
          Course & Version:
        </h2>
        <div style={{ display: "flex" }}>
          ...
          >
            ...
            {courses &&
              courses.map((courses) => (
                <option
                  key={`${courses.courseUuid}:${courses.versionNum}`}
                  value={`${courses.courseUuid}:${courses.versionNum}`}
                >
                  {`${courses.titleLbl} ${courses.codeLbl}, Version #${courses.versionNum}`}
                </option>
              ))}
          </select>
        </div>
