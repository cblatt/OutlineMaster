...
const OutlineForm = ({ courseOutline, department, course, previewOutline }) => {
  const navigate = useNavigate();
  const [preview, setPreview] = useState(false);
  const { register, handleSubmit, control, getValues } = useForm({
    defaultValues: courseOutline,
  });
  const { user } = useAuth();
    ...
    fetch(
      ...
      .then((res) => {
        fetch(process.env.REACT_APP_API_URI + "/editor-log", {
          ...
          .then(() => {
            navigate(
              `/prev-course-outline/${course.courseUuid}/${courseOutline.versionNum}`
            );
          });
      });
  }
...
export default function EditOutline() {
  ...
  const fetchData = useCallback(
    async function fetchData() {
      ...
      // Parse course topics to convert to select options
      data.courseTopics = data.courseTopics.map((topic) => ({
        ...topic,
        gaIndicators: topic.gaIndicators.map((indicator) => {
          const num = indicator.match(/\d+/g);
          const letr = indicator.match(/[a-zA-Z]+/g);
          const indicatorObj = {
            label: `${letr[0]} ${num[0]}`,
            value: indicator,
          };
